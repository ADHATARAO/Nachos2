...
//to allocate the memory for threads
#ifdef CHANGED
...
// in order to find the begin of the thread stack, if not enough memory then Assertion fault
#ifdef CHANGED
...
void AddrSpace::DealloateMapStack() {
	bitmapThreadStack->Clear(currentThread->GetThreadId());
}

static void ReadAtVirtual( OpenFile *executable, int virtualaddr,int numBytes, int position,TranslationEntry *pageTable,unsigned numPages)
{
	//TO go back to old pagetable
